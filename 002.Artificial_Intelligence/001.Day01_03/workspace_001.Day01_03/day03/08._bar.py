...
# -表示左移， + 表示右移， 0.2 是单个柱子的宽度
plt.bar(x - 0.2, apples, 0.2)
plt.bar(x, oranges, 0.2)
plt.bar(x + 0.2, pear, 0.2)
...
for i in range(len(x)):
    plt.text(x[i] - 0.2, apples[i], int(apples[i]), fontsize=6, ha='center', va='bottom')
    plt.text(x[i], oranges[i], int(oranges[i]), fontsize=6, ha='center', va='bottom')
    plt.text(x[i] + 0.2, pear[i], int(pear[i]), fontsize=6, ha='center', va='bottom')
