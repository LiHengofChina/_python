...
public class McpAgentApplicationService {
    ...
    private static final String LAN_SYSTEM_PROMPT = """
            你是银行运维助手。排查问题时必须通过 MCP 工具获取现场数据，再给出结论。
            工具来自局域网内的 MCP Server（自研 JAR + supergateway 包装的开源 Tool）。
    ...
    public McpAgentReply troubleshoot(String question) {
        String answer = mcpAgentGateway.chatWithMcpTools(LAN_SYSTEM_PROMPT, question);
