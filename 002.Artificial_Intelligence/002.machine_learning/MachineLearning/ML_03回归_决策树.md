...
//======================== 决策树（重要）决策树这种一次就能训练完成对吧，多次训练没有意义，除非样本数据增加
一次 fit() 就训练完成
没有“多轮训练（epoch）”的概念
再 fit() 一次 ≈ 重新训练一棵新树
...
//================================= feature_importances_ 是：
//================================= feature_importances_ 是：
	“模型在训练后，计算出“每个特征对分类的贡献程度”
假设输出是：
    [0.05 0.02 0.10 0.40 0.30 0.13]
   

这表示：
    特征编号	重要性
    0 avg_length	0.05
    1 fixed_length_flag	0.02
    2 avg_digit_ratio	0.10
    3 phone_regex_ratio	0.40
    4 id_regex_ratio	0.30
    5 birth_ratio	0.13
总和 = 1.0
...
if phone_regex_ratio > 0.8 → PHONE

每次分裂都会：

降低“混乱度”（Gini impurity）
哪个特征“让混乱度下降最多”，
