...
def vgg_bn_drop(image):
    ...
    #卷积块：
    def conv_block(input, num_filter, group): #卷积核数量、组数
        ...
        return fluid.nets.img_conv_group(input=input, #要卷的是谁#输入
                                  ...
                                  conv_act='relu',           #卷积层激活函数
                                  conv_with_batchnorm=True,  #是否做批量归一化
                                  pool_stride=2,             #池化步长
                                  pool_type='max'            #池化类型
                                  )
    ...
    drop = fluid.layers.dropout(x=fc1,
                                dropout_prob=0.5)
    fc2 = fluid.layers.fc(input=drop,
                          size=512 #神经元个数
                          )
    drop = fluid.layers.dropout(x=fc2,
                                dropout_prob=0.5)
...
#梯度下降
optimizer = fluid.optimizer.Adam(learning_rate=0.00001  # 学习率 vgg学习率
                                 )
